...
Many applications have regulatory requirements regarding security and
compliance including defense, finance and health care. These
requirements can be characterized in broad strokes as authentication
(only allowing people who are authorized to access certain resources
in), encryption (both at rest and in transit), auditability (track
users' access to the system and to resources) and vulnerability
mitigation.

In the architecture section, we describe how our cloud and container
architecture using an AWS environment with AWS's Elastic Container
Service (ECS), ensures encryption at rest and in transit of a
container and to the container and integrate access with an
application load balancer (ALB) for authentication.

In the applications and authentication section, we take a deeper dive
into the authentication provided by the ALB using JWT specifically to
integrate jupyter and RStudio.

In the security and compliance section, we address addition issues of
security and compliance such as the encryption of the underlying cloud
architecture, auditing capabilities, and mitigation of vulnerabilities.

Described in this paper is an implementation which satisfies privacy and
security concerns. However, the reader is advised to tailor the system to suit
one's own needs.
