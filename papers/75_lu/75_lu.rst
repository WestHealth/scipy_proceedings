...
Since the matrix manipulation operations and input/output shape definitions differ from package to package, we provide a qsoftmax implementation in several popular packages.
...
We used a separate ``Activation`` layer to make the use of the``qsoftmax`` metafunction as defined above clearer.
...
quasiorthonormal sets ``basis4`` and ``basis7`` and only took the first 10
