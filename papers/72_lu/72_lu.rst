...
On the container side, the environment variables `http_proxy` and `https_proxy` must be set to forward all http and https request to the EC2 instance at port 8888. Two methods can be used. Either we can add the environment variables to the task definition when an application service created or it can defined in the container's `Dockerfile` with the following lines:

.. code:: csh

ENV...

For the proxy we selected ``tinyproxy`` because it is lightweight and allows....
...
To supplement the auditing and monitoring capability a logging agent [ADRIAN: any specific name?], is installed on the EC2 instance. Essentially, this agent transmits logs of interest such as the system log ``syslog`` to an external log management system. Through this mechanism behaviours such as privlege escalation (e.g. ``sudo``) are tracked. 
...
There are many agents that can be installed.
ECS Agent
^^^^^^^^^
